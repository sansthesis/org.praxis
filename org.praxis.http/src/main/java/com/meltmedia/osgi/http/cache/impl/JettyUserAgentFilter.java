 ...
 * <dl>
 * <dt>attribute</dt>
 * <dd>If set, then the request attribute of this name is set with the matched user agent string</dd>
 * <dt>cacheSize</dt>
 * <dd>The size of the user-agent cache, used to avoid reparsing of user agent strings. The entire cache is flushed when this size is reached</dd>
 * <dt>userAgent</dt>
 * <dd>A regex {@link Pattern} to extract the essential elements of the user agent. The concatenation of matched pattern groups is used as the user agent name</dd>
 * <dl>
 * An example value for pattern is <code>(?:Mozilla[^\(]*\(compatible;\s*+([^;]*);.*)|(?:.*?([^\s]+/[^\s]+).*)</code>. These two pattern match the common compatibility user-agent strings and extract the real user agent, failing that, the first element of the agent string is returned.
 * 
 * 
 */
public abstract class JettyUserAgentFilter implements Filter {
  private Pattern _pattern;
  private final Map _agentCache = new ConcurrentHashMap();
  ...
  /* ------------------------------------------------------------ */
  /**
   * Get UserAgent. The configured agent patterns are used to match against the passed user agent string. If any patterns match, the concatenation of pattern groups is returned as the user agent string. Match results are cached.
  ...
  public String getUserAgent(final String ua) {
    ...
    if( tag == null ) {
      final Matcher matcher = _pattern.matcher(ua);
      if( matcher.matches() ) {
